...
def train_single(base_model_path: Path=None, device: str='cuda', reset_lr: bool=False, scheduler_type: str='cos'):
    device = torch.device(device)

    feature_num = len(feature_names)
    model = UNet(3 + feature_num * 3, 3).to(device)
    model = init_weights(model)
